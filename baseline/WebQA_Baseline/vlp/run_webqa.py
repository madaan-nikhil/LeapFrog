...
def main():
    ...
    parser.add_argument("--model_recover_path",
                        default=None,
                        # default="data/ckpts/cc_g8_lr1e-4_batch512_s0.75_b0.25/model.30.bin",
                        type=str,
    ...
    # webqa dataset (TODO: need to check, seems unused)
    ### ORIGINAL 
    # parser.add_argument('--txt_dataset_json_path', type=str, default=os.path.join(DATASET_PATH, "txt_dataset_0904_clean_fields.json"), help="the path to the txt dataset path if you have")
    # parser.add_argument('--img_dataset_json_path', type=str, default=os.path.join(DATASET_PATH, "img_dataset_0904_clean_fields.json"), help="the path to the img dataset path if you have") # org
    ### FULL TRAIN SET
    # parser.add_argument('--txt_dataset_json_path', type=str, default=os.path.join(DATASET_PATH, "WebQA_train_val.json"), help="the path to the txt dataset path if you have")
    # parser.add_argument('--img_dataset_json_path', type=str, default=os.path.join(DATASET_PATH, "WebQA_train_val.json"), help="the path to the img dataset path if you have")
    ### SMALL TRAIN SET FOR DEBUG
    ...
    parser.add_argument('--gold_feature_folder', type=str, default=os.path.join(DATASET_PATH, "imgFeatures_upd/gold"))
    parser.add_argument('--distractor_feature_folder', type=str, default=os.path.join(DATASET_PATH, "imgFeatures_upd/distractors"))
    parser.add_argument('--x_distractor_feature_folder', type=str, default=os.path.join(DATASET_PATH, "imgFeatures_x_distractors/x_distractors"))

    # parser.add_argument('--img_metadata_path', type=str, default="../../dataset/img_metadata-Copy1.json", help="how many samples should be loaded into memory")
    ...
    if args.do_train:
        ...
        for i_epoch in trange(start_epoch, args.num_train_epochs+1, desc="Epoch"):
            ...
            for step, loader_idx in enumerate(iter_bar):
                batch = next(dataloader_iters[loader_idx]) # TODO: 03/19/22 02:31, we are tired. Will wake up here.
